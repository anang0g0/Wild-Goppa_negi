...
# 20230831
複合できなかったりしてよくわからないところを修正。ulimit -sをしなくても動くようにパラメータを設定した。
動作確認は以下の通り。  
tokyo/marimo/vmo.c（奇標数の拡大体のGoppa符号をバーレカンプマッシー法で符号化と復号）  
tokyo/fujiyama/fujiyama.c（素体上のGoppa符号をピーターソン法で復号する）  
以上の2つを修正、動作確認した。
